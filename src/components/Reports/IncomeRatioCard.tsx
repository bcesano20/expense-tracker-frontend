...
export const IncomeRatioCard = ({ incomeRatio, loading, monthName, year }: IncomeRatioCardProps) => {
  const hasData = !loading && incomeRatio && (incomeRatio.incomeCount ?? 0) > 0

  const cfg = hasData
    ? (STATUS_CONFIG[incomeRatio!.status] ?? STATUS_CONFIG.BALANCED)
    : null
      ...
      {hasData && (
        <div className="border border-gray-200 rounded-lg p-3 sm:p-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 justify-between items-center">
            ...
            <span className={`text-xs font-medium px-2 py-1 rounded-full shrink-0 ${cfg!.bg} ${cfg!.text}`}>
              {cfg!.label}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            <div>
              ...
              <p className="text-xl font-bold text-green-600">${(incomeRatio!.totalIncome ?? 0).toFixed(2)}</p>
              <p className="text-xs text-gray-400">{incomeRatio!.incomeCount ?? 0} ingresos</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Gastos</p>
              <p className="text-xl font-bold text-red-600">${(incomeRatio!.totalExpenses ?? 0).toFixed(2)}</p>
              <p className="text-xs text-gray-400">{incomeRatio!.expenseCount ?? 0} gastos</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Balance</p>
              <p className={`text-xl font-bold ${(incomeRatio!.balance ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ${(incomeRatio!.balance ?? 0).toFixed(2)}
              </p>
              <p className="text-xs text-gray-400">{(incomeRatio!.expensePercentage ?? 0).toFixed(1)}% en gastos</p>
            </div>
          </div>
          ...
          <div>
            <div className="flex h-2 rounded-full overflow-hidden bg-gray-100">
              <div className="bg-green-500 h-full transition-all" style={{ width: `${incomePct}%` }} />
